...
state.context_menu_items = (function()
	...
	for line in io.lines(input_conf_path) do
		local key, command, title = string.match(line, ' *([%S]+) +(.*%S) *#menu: *(.*)')
		if key and not (key == '_' and command == 'ignore') and command ~= 'script-binding uosc/menu' then
			local is_dummy = key:sub(1, 1) == '_'
			local submenu_id = ''
